...
class UCO3DMotor(tfds.core.GeneratorBasedBuilder):
    ...
    def _info(self):
        return tfds.core.DatasetInfo(
            ...
            features=tfds.features.FeaturesDict({
                ...
                'mask': tfds.features.Image(shape=(None, None, 3)), #mask
    ...
    def _split_generators(self, dl_manager):
        path = '/kmh-nfs-ssd-us-mount/code/kristine/lvm/nvs_renders/motor'
        return {
            'train': self._generate_examples(img_dir=path+'/renders_motor/train', plucker_dir=path+'/plucker_motor/train', latents_dir=path+'/motor_train_latents'),
            'val': self._generate_examples(img_dir=path+'/renders_motor/val', plucker_dir=path+'/plucker_motor/val', latents_dir=path+'/motor_val_latents'),
        }
    ...
    def _generate_examples(self, img_dir, plucker_dir, latents_dir):
        ...
        for filename in sorted(os.listdir(img_dir)):
            if filename.lower().endswith('.png') and not filename.endswith('_plucker.png'):
                ...
                mask_latents_path = os.path.join(latents_dir, filename+'_mask.npy')
                motor_dir = os.path.dirname(os.path.dirname(img_dir))
                camera_json_path = os.path.join(motor_dir, 'camera_params.json')
                ...
                try:
                    ...
                    if not os.path.exists(image_latents_path):
                        assert False, f"Image latent file {image_latents_path} does not exist"
                    if not os.path.exists(mask_latents_path):
                        assert False, f"Mask latent file {mask_latents_path} does not exist"
                    yield filename, {
                        'image': img_path,
                        'mask': plucker_path,
                        'image_latents': np.load(image_latents_path),
                        'mask_latents': np.load(mask_latents_path),
                        'metadata': {
                            'filename': filename,
...
# data loader
# Returns batched dataset: each batch is (images, plucker_coordinates) with shapes (B, 256, 256, 3)
def load_data(dataset_name='UCO3DMotor', split='train', data_dir=None, batch_size=64, repeat=True, shuffle=True):
...
def sanity_check(ds_dir, split, num_batches=1, shuffle=False):
    ...
    for i, (images, masks, image_latents, mask_latents, metadata) in enumerate(dataloader.take(num_batches)):
        ...
        masks = masks[:15].numpy() if hasattr(masks, 'numpy') else np.asarray(masks[:15])
        masks_15 = np.clip(masks, 0, 255).astype(np.uint8)

        print(f"--- batch {i+1} ---")
        print(f"Image batch shape: {images.shape}")
        print(f"Mask batch shape: {masks.shape}")
        print(f"Image latent batch shape: {image_latents.shape}")
        print(f"Mask latent batch shape: {mask_latents.shape}")

        print(f"Image max/min: {np.max(images)}, {np.min(images)}")
        print(f"Mask max/min: {np.max(masks)}, {np.min(masks)}")
        print(f"Image latent max/min: {np.max(image_latents)}, {np.min(image_latents)}")
        print(f"Mask latent max/min: {np.max(mask_latents)}, {np.min(mask_latents)}")
...
def main():
    ds_dir = '/kmh-nfs-ssd-us-mount/code/kristine/lvm/uco3d_motor_tfds/'
