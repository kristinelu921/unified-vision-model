...
class Mass13k(tfds.core.GeneratorBasedBuilder):
    ...
    def _split_generators(self, dl_manager):
        path = '/kmh-nfs-ssd-us-mount/code/kristine/lvm/mass13k_data' #TODO: replace with GSBUCKET location (us: central2)
...
def load_data(dataset_name='Mass13k', split='train', data_dir=None, batch_size=64, repeat=True, shuffle=True):
    ...
    ds = ds.map(lambda ex: (ex['image'], ex['mask']))
    if shuffle:
...
def preprocess_func(image, mask): 
...
def sanity_check(ds_dir, split, num_batches=1, shuffle=False):
    ...
    for i, (images, masks) in enumerate(dataloader.take(num_batches)):
...
def single_pass_thru():
    ...
    with open('single_pass_thru.txt', 'w') as f:
        for image, mask in ds:
            f.write(f"{image.shape}, {mask.shape}\n")
