from Mass13k import load_data, sanity_check

def one_pass():
    ds_dir = '/kmh-nfs-ssd-us-mount/data/kristine/lvm/mass13k_tfds' #OR: if using a DIFFERENT ZONE (not us-central2), use '/mnt/klum/data'  (after you run the warmup script), see commands.txt for example
    ...
    with open('/kmh-nfs-ssd-us-mount/code/kristine/lvm/single_pass_thru.txt', 'w') as f:
        pass
    for i, (images, masks) in enumerate(ds):
        ...
        with open('/kmh-nfs-ssd-us-mount/code/kristine/lvm/single_pass_thru.txt', 'a') as f: #current command: write out the files
            f.write(f"{i}, {images.shape}, {masks.shape}\n")
